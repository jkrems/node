...
bool AgentImpl::StartIoThread() {
  ...
  Local<Value> emit_fn = process_object->Get(OneByteString(isolate, "emit"));
...
#ifdef __POSIX__
void AgentImpl::DebugProcess(int pid) {
  int r = kill(pid, SIGUSR1);
  if (r != 0) {
    return parent_env_->ThrowErrnoException(errno, "kill");
  }
}
#endif  // __POSIX__
...
void AgentImpl::DebugProcess(int pid) {
  ...
  if (GetDebugSignalHandlerMappingName(pid,
                                       mapping_name,
                                       arraysize(mapping_name)) < 0) {
    parent_env_->ThrowErrnoException(errno, "sprintf");
    goto out;
  }
...
void Agent::DebugProcess(int pid) {
  CHECK_NE(impl->client(), nullptr);
  impl->DebugProcess(pid);
}
