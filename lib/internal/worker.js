...
class Worker extends EventEmitter {
  constructor(filename, options = {}, $) {
    ...
    const url = options.eval || internal ? null : pathToFileURL(filename);
    ...
    let outsideBelowPort, insideBelowPort;
    if (!internal) {
      let loaderRPC = require('internal/modules/esm/loader').getBottomLoaderRPC();
      if (loaderRPC) {
        ({
    ...
    this[kPort].postMessage({
      ...
      port2,
      ...(outsideBelowPort ? [outsideBelowPort] : []),
...
function InternalWorker(a, b) {
  return new Worker(a, b, InternalWorker);
}
