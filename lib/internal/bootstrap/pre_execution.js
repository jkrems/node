...
function prepareMainThreadExecution(expandArgv1 = false) {
  ...
  initializePolicy();
  // Loaders need policy and spin up threads so start them ASAP
  const bottomLoader = startLoaders();
  ...
  initializeWASI();
  initializeCJSLoader();
  initializeESMLoader(bottomLoader);
...
function startLoaders() {
  ...
  const userLoader = InternalWorker('internal/modules/esm/worker', {
    // stdout: true,
    // strerr: true,
    workerData: {
      loaderHREF: `data:text/javascript;base64,${
        Buffer.from(`
        export async function resolve (o) {
          console.dir({o});
          const par = await parent.resolve(o);
          console.dir({par});
          return {
            locationHREF: par.locationHREF,
            contentType: 'module'
          }
        }
